...
class PaintSettingsDialog(QDialog):
    ...
    def accept(self, /) -> None:
        ...
        viewer.painter_radius = vals['radius']
        viewer.painter_depth = vals['depth']
        viewer.set_viewer_mode(ViewerMode.PAINT)
        return super().accept()
...
def create_paint_settings_dialog(lumen_core: Lumen):
    selected_segment = lumen_core.get_selected_segment()

    if not selected_segment:
        return

    dialog = PaintSettingsDialog(lumen_core)
    dialog.exec()
