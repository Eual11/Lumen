...
class ActorTableModel(QAbstractTableModel):
    def __init__(self, data_owner: LumenCore.Lumen):
        ...
        self.actor_dict = data_owner.renderer.actors  # {vtkActor: ActorInfo}
    ...
    def data(self, index: QModelIndex, role=Qt.DisplayRole):
        ...
        actor, info = self._data[row]

        if role == Qt.DisplayRole:
            if col == 0:
                return getattr(info, "name", f"Actor {row+1}")
            elif col == 1:
    ...
    def setData(self, index, value, role):
        ...
        if role == Qt.EditRole:
            if col == 0:
                ...
                self.data_owner.renderer.set_actor_transform(actor, value)
                self.dataChanged.emit(index, index)
                return True
            elif col == 2:
                info['visible'] = value
                self.data_owner.renderer.set_actor_visibility(actor, value)
                self.dataChanged.emit(index, index)
                return True
    ...
    def update_model(self):
        self._data = list(self.actor_dict.items())
...
class ActorsTableWidget(QWidget):
    def __init__(self, data_owner, selection_callback=None, parent=None):
        ...
        # Table Model
        self.model = ActorTableModel( self.data_owner)
